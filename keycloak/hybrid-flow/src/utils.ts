...
export function makeLoginUrl() {
  ...
  return `http://localhost:8000/realms/fc-realm/protocol/openid-connect/auth?${loginUrlParams.toString()}`;
}
...
export function exchangeCodeForToken(code: string) {
  ...
  return fetch(
    "http://localhost:8000/realms/fc-realm/protocol/openid-connect/token",
    {
      method: "POST",
      headers: {
...
export function makeLogoutUrl() {
  ...
  return `http://localhost:8000/realms/fc-realm/protocol/openid-connect/logout?${logoutParams.toString()}`;
}
